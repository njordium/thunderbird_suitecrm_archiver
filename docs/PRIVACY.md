...
- **When a reply has lost its case number** and case matching is on, the `Message-ID` of up
  to five messages this one is a reply to, so the CRM can say whether it stored one of them
  against a Case. A Message-ID is an opaque identifier generated by a mail server, not
  content: no subject, no body and no address goes with it. The CRM already holds these for
  any mail it sent or filed, which is exactly why they can be matched
  (`findCaseByReferences` in `src/lib/caseRef.js`).
...
- **No third-party site is contacted, ever.** A website guessed from a sender's email domain
  is filled in for you to check and is never fetched. The add-on holds one host permission,
...
Seven entries in Thunderbird's extension storage, inside your Thunderbird profile
...
| `moduleTrouble` | Which CRM modules refused a search, with the HTTP status and the CRM's own error text, so Preferences can show why one was turned off. Module names and an error string, nothing about you or your mail. Cleared by *Re-scan modules* |
